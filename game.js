...
function renderEndingGallery() {
  ...
  // 見出しの真下の行に小さく添える(display:block は CSS 側)。1行ぶん背が伸びるので、
  // ボタンと図鑑の上の余白を詰めて取り戻してある —— 詰めないと低い端末で
  // 図鑑の最下段が画面の外へ出る。「全達成！」は同じ行のままにするため、後ろに足す。
...
const BGM_BY_KEY = {
  ...
  // 最終日の昼(E20C)にも同じ曲を戻す —— 以前は E20B と同じ quiet1 にして
  // 「続きの場面」を音で示していたが、初日の夜と同じ絵・同じ曲になり、
  // 日付が変わったと分からなかった(2026-09-12 本人指摘)。祭りの真っ最中に
  // 本番の曲が返り、E21 で quiet2 に落ちることで「祭りが終わる」も効く
  E20: "festival",
  E20C: "festival",
...
const SPEAKER_BY_LINE = new Map([
  ["「時間になりました。予定通り文化祭実行委員は集まってください」", "broadcast"],
  ["「あ、ちょうどええわ。そっちの机、こっち持ってきてくれる?」", "unknown"],
  // E1 で名乗る前の3行。既定のまま(ハナエ)にすると、名札が
  // 「それが味村ハナエとの、最初の会話だった」より先に名前を割ってしまう
  ["「えーっと、初めましてやんな?」", "unknown"],
  ["「あ、こないだ転校してきた人や!」", "unknown"],
